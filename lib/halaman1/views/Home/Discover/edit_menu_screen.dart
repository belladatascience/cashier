...
class _EditMenuScreenState extends State<EditMenuScreen> {
  ...
  Map<String, List<Map<String, dynamic>>> get _categoryDataMap =>
      widget.categoryDataMap ?? MenuDataStore.instance.categoryDataMap;
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Nama Menu',
                      hintText: 'Contoh: Caramel Waffle',
                      labelStyle: GoogleFonts.workSans(color: colorOutline),
                      border: const OutlineInputBorder(),
                    ),
                  ),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Harga',
                      hintText: 'Contoh: 25000 atau Rp 25.000',
                      labelStyle: GoogleFonts.workSans(color: colorOutline),
                      border: const OutlineInputBorder(),
                    ),
                  ),
                ...
                onPressed: () {
                  ...
                  if (name.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          'Mohon masukkan nama menu terlebih dahulu!',
                        ),
                  ...
                  final catName = _selectedTab < _categoryNames.length
                      ...
                      : 'Food';
                  final priceDigits = rawPrice.replaceAll(RegExp(r'[^\d]'), '');
                  final parsedPrice = priceDigits.isNotEmpty
                      ? (int.tryParse(priceDigits) ?? 20000)
                      : 20000;
                  final formattedPriceText =
                      'Rp ${parsedPrice.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]}.')}';
                  ...
                  MenuDataStore.instance.addMenuItem(catName, newMenuItem);
                  if (widget.categoryDataMap != null &&
                      widget.categoryDataMap !=
                          MenuDataStore.instance.categoryDataMap) {
                    if (!widget.categoryDataMap!.containsKey(catName)) {
                      widget.categoryDataMap![catName] = [];
                    }
                    widget.categoryDataMap![catName]!.add(newMenuItem);
                  }
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Menu "$name" ($formattedPriceText) berhasil ditambahkan ke kategori $catName! 🎉',
                      ),
                      backgroundColor: colorSecondary,
                      behavior: SnackBarBehavior.floating,
                    ),
                              ...
                              child: Center(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Ketuk tombol "+ New Menu" di bawah untuk menambahkan menu pertama.',
                                      textAlign: TextAlign.center,
                                      style: GoogleFonts.workSans(
                                        fontSize: 13,
                                        color: colorOnSurfaceVariant,
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (catName.isNotEmpty) {
                MenuDataStore.instance.addCategory(catName);
                if (widget.categoryNames != null &&
                    !widget.categoryNames!.contains(catName)) {
                  widget.categoryNames!.add(catName);
                }
                if (widget.categoryDataMap != null &&
                    !widget.categoryDataMap!.containsKey(catName)) {
                  widget.categoryDataMap![catName] = [];
                }
        ...
        actions: [
          if (_categoryNames.length > 1)
            TextButton(
              onPressed: () {
                Navigator.pop(dContext);
                MenuDataStore.instance.deleteCategory(oldName);
                widget.categoryNames?.remove(oldName);
                widget.categoryDataMap?.remove(oldName);

                setState(() {
                  if (_selectedTab >= _categoryNames.length) {
                    _selectedTab = _categoryNames.isEmpty
                        ? 0
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (newName.isNotEmpty && newName != oldName) {
                MenuDataStore.instance.renameCategory(oldName, newName);
                if (widget.categoryNames != null) {
                  final idx = widget.categoryNames!.indexOf(oldName);
                  if (idx != -1) widget.categoryNames![idx] = newName;
                }
                if (widget.categoryDataMap != null) {
                  final items = widget.categoryDataMap!.remove(oldName) ?? [];
                  widget.categoryDataMap![newName] = items;
                }
